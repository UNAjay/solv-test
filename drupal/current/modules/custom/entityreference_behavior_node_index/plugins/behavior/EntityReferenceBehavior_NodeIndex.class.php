...
/**
 * @file
 * CTools plugin class for the node-index behavior.
 */

/**
 * Extends an entityreference field to maintain its references to nodes
 ...
 */
class EntityReferenceBehavior_NodeIndex extends EntityReference_BehaviorHandler_Abstract {
  ...
  public function access($field, $instance) {
    if ($instance['entity_type'] != 'taxonomy_term' || $field['settings']['target_type'] != 'node') {
      return;
    }
   ...
   */
  protected function buildNodeIndex($term) {
    ...
    if ($status) {      
      ...
      $tid_all = array();
      foreach (field_info_instances('taxonomy_term', $term->vocabulary_machine_name) as $instance) {
        $field_name = $instance['field_name'];
        $field = field_info_field($field_name);
        if (!empty($field['settings']['target_type']) && $field['settings']['target_type'] == 'node' && $field['storage']['type'] == 'field_sql_storage') {
          // If a field value is not set in the node object when node_save() is
          // called, the old value from $node->original is used.
          if (isset($term->{$field_name})) {
            $items = $term->{$field_name};
          }
          elseif (isset($term->original->{$field_name})) {
            $items = $term->original->{$field_name};
          }
          else {
            continue;
          }
          foreach (field_available_languages('taxonomy_term', $field) as $langcode) {
        ...
        if ($field['module'] == 'taxonomy' && $field['storage']['type'] == 'field_sql_storage') {
          ...
          foreach (field_available_languages('taxonomy_term', $field) as $langcode) {
            if (!empty($items[$langcode])) {
              foreach ($items[$langcode] as $item) {
                $original_tid_all[$item['nid']] = $item['nid'];
              }
            }
          }
        }
      ...
      // remove all references to nodes from this taxonomy - if any reference appear, it will be added
      db_delete('taxonomy_index')->condition('tid', $term->tid)->execute();
      
      // Insert index entries for all the node's terms.
      if (!empty($tid_all)) {
        $query = db_insert('taxonomy_index')->fields(array('nid', 'tid', 'sticky', 'created'));
        foreach ($tid_all as $nid) {
          $query->values(array(
            'nid' => $nid,
            'tid' => $term->tid,
            'sticky' => 0,
  ...
  public function settingsForm($field, $instance) {
    ...
    if ($target != 'node') {
      $form['ti-on-terms'] = array(
        '#markup' => t('This behavior can only be set when the target type is node, but the target of this field is %target.', array('%target' => $target)),
      );
    }
    ...
    if ($entity_type != 'taxonomy_term') {
      $form['ti-on-nodes'] = array(
        '#markup' => t('This behavior can only be set when the entity type is taxonomy_term, but the entity type of this instance is %type.', array('%type' => $entity_type)),
      );
    }
